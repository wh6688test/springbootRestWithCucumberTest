...
Feature: any test
  ...
  Scenario: scenario 1
   ...
   Scenario: scenario 2
    # Enter steps here
   Given the group with gid 2 is created
   And api request uri '/group/member/rating' and expected status code 200
   When I update the group member:
     |gid|memberId|rating|
     |2  |3       |5     |
   Then I retrieve the group list with group member id '3' via api '/group/member' with expected response status 200
   And I verify that the group response with members matches:
    |id  |rating|
    |3 |5|
   Then I delete the group with gid 2 via api "/group" with response status 204
